...
dis_file <- "./Figure_S12/Family_single_pm_dis.txt"
...
write.table(dis_merge, "Figure_S12b.txt",
...
######## for network cluster ##################################################
dis_file <- "./Figure_S12/Cluster_single_pm_dis.txt"
...
write.table(dis_merge, "Figure_S12a.txt",
            quote = F, sep = "\t", row.names = F)
...
p <- plot_grid(p_a, p_b, nrow = 2, align = "v", axis = "l", labels = 'auto',
               rel_heights = c(1, 1.2))
ggsave("Figure_S12.pdf", p, width = 10, height = 7)
